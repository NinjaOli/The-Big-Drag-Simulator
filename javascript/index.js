...
class MiniChallenge {
    generateDescription() {
        document.body.style.backgroundImage = "url('image/cube.webp')";
        ...
        if (totalCastSize >= 10 && currentCast.length == 7 && !all_stars && !lipsync_assassin && !all_winners && episodeCount > 3 && !readingCheck || currentCast.length == totalCastSize && (all_stars || lipsync_assassin) && !readingCheck || episodeCount == 1 && all_winners && !readingCheck) {
            ...
            description.innerHTML = "The library is open! In this week's mini-challenge, the queens will read eachother!";
            readingCheck = true;
            readingChallenge();
        }
...
function miniChallenge() {
   ...
   if (premiereCounter == 3 && s14Premiere) {
    ...
    if (snatchonly == true) {
		...
		miniChallengeScreen.createButton("Proceed To Performances", "secretcelebrity()");
		

        
    }
    else if (runwayonly == true) {
		miniChallengeScreen.createButton("Proceed", "runwaysmackdown()");
		

        
    }
    else if (rupaulm == true) {
		miniChallengeScreen.createHorizontalLine();
        miniChallengeScreen.createBigText(" Choose the Maxi-Challenge:");
		...
		miniChallengeScreen.createButton("Roast", "comedyChallenge()");
...
class ActingChallenge {
    generateDescription() {
        ...
        (function (desc2) {
            ...
            desc2[desc2["9021-HO."] = 15] = "90 Degrees.";
            ...
            desc2[desc2["Screech!."] = 37] = "There Is No Escape.";
            desc2[desc2["Whodunnit."] = 38] = "Summer Once Again.";
            ...
            desc2[desc2["Exorsisters."] = 47] = "A Trial To Watch.";
        })(desc2 || (desc2 = {}));
        description.innerHTML = "This week, the queens will take part in an acting challenge called " +  desc2[randomNumber(0, 47)];
    }
    rankPerformances() {
        for (let i = 0; i < currentCast.length; i++) {
...
class ComedyChallenge {
    generateDescription() {
        document.body.style.backgroundImage = "url('image/roast.webp')";
        let description = document.querySelector("b#Description");
        let desc1;
        let whatChallengeIs;
...
function runwayChallenge() {
    ...
    challenge.generateDescription();
    challenge.rankPerformances();
    let shoot = currentCast.filter(function (queen) { return queen.performanceScore < 6; });
    let toot = currentCast.filter(function (queen) { return queen.performanceScore >= 6 && queen.performanceScore < 16; });
    let softtoot = currentCast.filter(function (queen) { return queen.performanceScore >= 16 && queen.performanceScore < 26; });
    let boot = currentCast.filter(function (queen) { return queen.performanceScore >= 26; });
    createRunwayDesc(shoot, toot, softtoot, boot);
...
function Impersonation() {
    ...
    screen.createBigText("The queens talk with Rupaul about their snatch game characters...");
    for (let i = 0; i < currentCast.length; i++) {
		let snatch = randomNumber(0, snatchToDo.length - 1);
        screen.createImage(currentCast[i].image);
        screen.createBold(`${currentCast[i].getName()} will do a ${snatchToDo[snatch]} impersonation!!`);
    }
}
...
function runwaysmackdown(){
let challengeScreen = new Scene();
challengeScreen.clean();
challengeScreen.createHeader("Runway!")
challengeScreen.createParagraph("", "Description");
let challenge = new smackdownrunway();
challenge.generateDescription();
challenge.rankPerformances();
isDesignChallenge = true;
    queensPerformances()
...
class smackdownrunway {
    generateDescription(){
        ...
        description.innerHTML = "It's time for the queens to bring it to the runway! This week's theme is: " + desc[randomNumber(0, 59)];
    }
    rankPerformances() {
        for (let i = 0; i < currentCast.length; i++)
            currentCast[i].getRunwayChallenge();
...
class lipsyncs {
    generateDescription() {
        ...
        let description = document.querySelector("p#Description");
        description.innerHTML = "Its time for the queens to perform!";
    }
    rankPerformances() {
        for (let i = 0; i < currentCast.length; i++)
            currentCast[i].getTalentShow();
...
function queenSongs() {
    ...
    performancescreen.createBigText("Queen's Song Choices");
    for (let i = 0; i < currentCast.length; i++) {
        let song = randomNumber(0, lsSongs.length - 1);
        performancescreen.createImage(currentCast[i].image);
        performancescreen.createBold(`${currentCast[i].getName()} will lip-sync to: ${lsSongs[song]} `);
        let score = randomNumber(0, performances.length - 1);
        performancescreen.createBold(`${currentCast[i].getName()} had a ${performances[score]} `);
        let snatch = randomNumber(0, snatchToDo.length - 1);
        performancescreen.createBold(` They are guessed to be: ${snatchToDo[snatch]} `);
    }
}
let performances = [
    "good performance",
...
function runway() {
    ...
    let shoot = currentCast.filter(function (queen) { return queen.runwayScore < 6; });
    let toot = currentCast.filter(function (queen) { return queen.runwayScore >= 6 && queen.runwayScore < 16; });
    let softtoot = currentCast.filter(function (queen) { return queen.runwayScore >= 16 && queen.runwayScore < 26; });
    let boot = currentCast.filter(function (queen) { return queen.runwayScore >= 26; });
    createRunwayDesc(shoot, toot, softtoot, boot);
...
function createChallenge(challenges, miniChallengeScreen) {
    for (let i = 0; i < currentCast.length; i++){
     ...
     if (rupaulm == true || secret == true || snatchonly == true || runwayonly == true) {

    

     }
    else if (currentCast.length == totalCastSize && regularFormat && episodeCount == 1 && s6Premiere == false || currentCast.length == totalCastSize && thailandFormat && episodeCount == 1 && s6Premiere == false || currentCast.length == totalCastSize && team || currentCast == firstCast && s6Premiere || currentCast == secondCast && s6Premiere)
...
function createRunwayDesc(shoot, toot, softtoot, boot) {
    let screen = new Scene();
    if (shoot.length !== 0) {
        for (let i = 0; i < shoot.length; i++) {
            screen.createImage(shoot[i].image, "darkblue");
            shoot[i].runwayScore = 10;
        }
        screen.createBold("", "shootR");
        let shootText = document.getElementById("shootR");
        for (let i = 0; i < shoot.length; i++)
            shootText.innerHTML += `${shoot[i].getName()}, `;
        shootText.innerHTML += "shooted this runway!";
    }
    if (toot.length !== 0) {
        for (let i = 0; i < toot.length; i++) {
            screen.createImage(toot[i].image, "royalblue");
            toot[i].runwayScore = 5;
        }
        screen.createBold("", "tootR");
        let tootText = document.getElementById("tootR");
        for (let i = 0; i < toot.length; i++)
            tootText.innerHTML += `${toot[i].getName()}, `;
        tootText.innerHTML += "tooted this runway!";
    }
    if (softtoot.length !== 0) {
        for (let i = 0; i < softtoot.length; i++) {
            screen.createImage(softtoot[i].image);
            softtoot[i].runwayScore = 0;
        }
        screen.createBold("", "softtootR");
        let softtootText = document.getElementById("softtootR");
        for (let i = 0; i < softtoot.length; i++)
            softtootText.innerHTML += `${softtoot[i].getName()}, `;
        softtootText.innerHTML += "soft tooted this runway.";
    }
    if (boot.length !== 0) {
        for (let i = 0; i < boot.length; i++) {
            screen.createImage(boot[i].image, "pink");
            boot[i].runwayScore = -3;
        }
        screen.createBold("", "bootR");
        let bootText = document.getElementById("bootR");
        for (let i = 0; i < boot.length; i++)
            bootText.innerHTML += `${boot[i].getName()}, `;
        bootText.innerHTML += "booted this runway...";
    }
}
...
let rupaulm = false;
let secret = false;
let snatchonly = false;
let runwayonly = false;
function newEpisode() {
...
function predefCast(cast, format, finale, premiere = '', returning = '') {
    ...
    if (document.getElementById("riggoryLipsync").checked == true)
        riggoryLipsync = true;
        if (document.getElementById("secret").checked == true)
        secret = true; 
    if (document.getElementById("snatchonly").checked == true)
        snatchonly = true; 
        if (document.getElementById("runwayonly").checked == true)
        runwayonly = true;      
...
let donna = new Queen("Donna Bellissima", 7, 7, 7, 7, 7, 7, 7, "Donna");
let electra = new Queen("Electra Owl", 7, 7, 7, 7, 7, 7, 7, "Electra");
let fab = new Queen("Fabulosity", 7, 7, 7, 7, 7, 7, 7, "Fab");
let jackieW = new Queen("Jackie Would", 7, 7, 7, 7, 7, 7, 7, "JackieW");
let milli = new Queen("Milli Von Sunshine", 7, 7, 7, 7, 7, 7, 7, "Milli");
...
let lsSongs = ["Supermodel by RuPaul",
...
"Fright Song by Monster High",
"Boyfriend by Dove Cameron",
"Hold Me Closer by Cornelia Jakobs",
"Bimbo Doll by Tile Tsoli",
"Mayhem by Halestorm",
...
"Dancing Queen by ABBA",
"This Is How To Be A Heartbreaker by MARINA",
"You Won't Forget Me by La Bouche",
"Doo Woop by Lauryn Hill",
"Number One by Pabllo Vittar",
...
"Unspeakable Joy (Razor n Guido Radio Mix) by Kim English",
...
"BOYSH*IT by Madison Beer",
"Telepath by Conan Gray",
"Rich Girl by Gwen Stefani",
"Woman Like Me by Little Mix",
"Take Your Mama by Scissor Sisters",
"What Do I Tell My Friends by Bree Runway",
...
"Daddy Lessons by Beyonce ft. The Chicks",
...
"The Great Hope Design by Sevdaliza",
...
"Too Funky by George Michaels",
"Follow Me by Pabllo Vittar & Rina Sawayama",
"Fake Smile by Ariana Grande",
"Astronomy by Conan Gray",
"Brown Cow Stunning by Mo Heart",
"Poljubi Me Za Kraj by Silente",
"La La Love by Albina",
"Disko by LPS",
"Roman Holiday by Nicki Minaj",
"Mi Amor by Nova Miller",
"I Got It Bad by Addison Rae",
"Downtown by Macklemore",
"Best Friend by Saweetie ft Doja Cat",
"1 Thing by Amerie",
"Motorcycle by Slayyyter",
"Come Through by Pryianka & Lemon",
"Radio by Beyoncé",
"Loco Loco by Hurricane",
"Femme Fatale by Genevieve Waite",
"Somebody Else by The 1975",
"Heather by The AAA Girls",
"XXX by Kim Petras",
...
"911 by Lady Gaga",
"All Eyes On Me by Julie and The Phantoms",
"Arrasando by Thalia",
"A Public Affair by Jessica Simpson",
"Terra by Tanxugueiras",
"Fergalicious by Fergie",
"Killshot by Magdalena Bay",
"Cosmic Colors by Trixie Mattel",
"Pussy Talk by City Girls",
...
"Scandalous (With Tinashe) by Griffin and Tinashe",
"Leng by Shygirl",
...
"Body Language by Heidi Montag",
"Upgrade U by Beyonce & Jay-Z",
...
let lipsyncsEventsBad = [
    ...
    {event: "knocked their opponent off of the stage!", penalization: -7},
    {event: "doesn't have the right outfit for this lip-sync.", penalization: -2},
    {event: "just isn't slaying.", penalization: -3},
    {event: "hasn't moved at all since the song started!.", penalization: -7}
];
let lipsyncsEventsGood = [
    ...
    {event: "did a cartwheel into a split which wows the judges!!!.", points: 10},
    {event: "pretended to leave the stage.", points: 3},
    {event: "did a flawless death drop.", points: 5},
    {event: "has exactly the right outfit for this lip-sync.", points: 2},
    {event: "is giving lip-sync assassin energy!.", points: 5}
...
bottomQueens.sort((a, b) => b.votes - a.votes);
top2.push(assassin);
screen.createBold("The lip-sync assassin is... ");
screen.createButton("Ruveal Lip-Sync Assassin", "lsaruveal()", "lsaruveal" );
}

function lsaruveal(){
let button = document.querySelector("button#lsaruveal");
button.remove();
let screen = new Scene();
screen.createImage(assassin.image, "royalblue");
screen.createBold(assassin.getName() + " ! ");
